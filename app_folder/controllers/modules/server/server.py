# -*- coding: utf-8 -*-
from ..db_sqlite.models.model_forms import *
...
class Server:
    ...
    def __init__(self) -> None:
        self.app = FastAPI(docs_url="/docs",)
        self._setup_routes()
    ...
    def _setup_routes(self):
        @self.app.post("/add-query/")
        async def add_query(
        query: Annotated[Query, Body()],
        session: Session = Depends(self.get_db)
    )-> Any:

            # getting an query if exists
            query_get = session.get(QueryDB, query.id)
            if query_get:
                ...
                try:
                    ...
                    if query.id is None:
                        id = uuid4()
                        query.id = id
                        db_query = QueryDB.model_validate(query)
                        ...
                        for order in orders:
                            ...
                            if order.items:
                                ...
                                for item in order.items:
                                    ...
                                    item_get = session.get(ItemDB, item.id)
                                ...
                                for item in order.items:
                                    ...
                                    if item.optional_parameters:
                                        ...
                                        for optional_parameter in item.optional_parameters:
                                            ...
                                            op_get = session.get(OptionalParametersDB, optional_parameter.id)
                                            ...
                                            if not op_get:
                                                ...
                                                if not optional_parameter.id:
                                                    ...
                                                    db_op = OptionalParametersDB.model_validate(optional_parameter)
    ...
    # Удаление запроса
        @self.app.delete("/delete-query/{query_id}")
        async def delete_query(
            query_id: Annotated[UUID, Path()],
            session: Session = Depends(self.get_db)
        ):
            query = session.get(QueryDB, query_id)
            if not query:
        ...
        @self.app.delete("/delete-order/{order_id}")
        async def delete_order(
            order_id: Annotated[UUID, Path()],
            session: Session = Depends(self.get_db)
        ):
            get_order = session.get(OrderDB, order_id)
        ...
        @self.app.delete("/delete-item/{item_id}")
        async def delete_item(
            item_id: Annotated[UUID, Path()],
            session: Session = Depends(self.get_db)
        ):
            get_item = session.get(ItemDB, item_id)
        ...
        @self.app.delete("/delete-op/{op_id}")
        async def delete_op(
            op_id: Annotated[UUID, Path()],
            session: Session = Depends(self.get_db)
        ):
            get_op = session.get(OptionalParametersDB, op_id)
    ...
    # Обновление запроса
                            ...
                            for parameter in item.optional_parameters:
                                if not parameter.id:
                                    try:
                                        id = uuid4()
                                        parameter.id = id
                                        db_parameter = OptionalParametersDB.model_validate(parameter)
                                    ...
                                    parameter_get = session.get(OptionalParametersDB, parameter.id)
        ...
        async def get_queryes(
            *,
            ...
            queryes = session.exec(select(QueryDB).offset(offset).limit(limit)).all()
            return JSONResponse(status_code=200, content=jsonable_encoder(queryes))

        @self.app.get("/get-orders/{query_id}")
        async def get_orders(
            ...
            if not query:
                ...
                orders = session.exec(select(OrderDB).where(OrderDB.query_id == query_id)).all()
                return JSONResponse(status_code=200, content=jsonable_encoder(orders))
        
        @self.app.get("/get-query/{q_id}")
        async def get_query(*,
            session: Session = Depends(self.get_db),
            q_id: Annotated[UUID, Path()]
        ):
            query_get = session.get(QueryDB, q_id)
        ...
        @self.app.get("/get-order/{order_id}")
        async def get_order(
            order_id: Annotated[UUID, Path()],
            session: Session = Depends(self.get_db)
        ):
        ...
        @self.app.get("/get-items/{order_id}")
        async def get_items(
        order_id: UUID,
        session: Session = Depends(self.get_db)
        ):
            ...
            if not order:
                ...
                items = session.exec(select(ItemDB).where(ItemDB.order_id == order_id)).all()
                items_op = []
                for item in items:
                    optional_parameters = session.exec(select(OptionalParametersDB).where(OptionalParametersDB.item_id == item.id)).all()
